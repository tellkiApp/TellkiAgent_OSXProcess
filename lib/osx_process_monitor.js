...
* Version 1.0
*
* DESCRIPTION: Monitor OSX Processes
*
* SYNTAX: node osx_process_monitor.js <METRIC_STATE> <PROCESSES>
* 
* EXAMPLE: node osx_process_monitor.js "1,1,1,1,1" "ssh service;/usr/sbin/sshd#1,node service;/usr/local/bin/node#3"
*
* README:
...
function monitorInputProcess(args)
{
    if (args[0] != undefined && args[1] != undefined && args[2] != undefined)
    {
        ...
        monitor(parseProcesses(args[1], args[2]));
    }
}

function parseProcesses(arg, ids)
{
    var i = 0;
    var out = [];
...
function monitor(processes)
{
    ...
    ls.on('exit', function (code) {
		if (code === 127)
	   		errorHandler(new UnableToGetMetricsError('Command \'ps\' not found.'));
		else if (code !== 0)
